...
行番号は `agent_support_example.py` の実装（本書作成時点）に対応。
...
| 「住民税の**減免**制度の概要を教えて」（keyword-trap） | **S5** | 第 1 段は `減免` に一致するが第 2 段が `question` → **誤爆抑止**して通常フロー継続 → answer |

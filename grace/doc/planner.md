...
| 4 | LLMを用いた実行計画の自動生成 | `planner.py` | `_create_llm_plan()`（Anthropic Claude を compat 経由で呼び出し） |
...
| 7 | フォールバック計画の提供 | `planner.py` | `_create_fallback_plan()` / `_create_rule_based_plan()` |
...
| `Planner.__init__()` | コンストラクタ（設定・モデル名・LLMクライアント・実行メモリ・キーワード抽出器の初期化） |
...
4. 単純クエリは `_create_rule_based_plan()` でLLM呼び出しなしの2ステップ計画を即時生成（`_prioritized_collection()` で優先コレクションを反映）
5. 複雑クエリ・Web検索指示クエリは `_create_llm_plan()` でAnthropic Claudeに計画生成を依頼
6. LLM計画では `_get_available_collections()` でQdrantから利用可能なコレクションを取得しプロンプトに埋め込む
7. レスポンスのJSONを `ExecutionPlan` にパースし、依存関係を検証して返却
8. 失敗時は `_create_fallback_plan()` が安全な代替計画を返す
...
classDef default fill:#000,stroke:#fff,color:#fff
classDef subgraphStyle fill:#1a1a1a,stroke:#fff,color:#fff
class PROMPT,CPROMPT,MARKERS,AMBIG,ISAMBIG,INIT,CREATE,SHOULD,PRIOR,RULE,LLMPLAN,CLARIFY,FALLBACK,GETCOLL,EST,ESTLLM,REFINE,CREATEP default
...
| メソッド | 概要 |
|---------|------|
| `__init__(config, model_name)` | 設定・モデル名・LLMクライアント・実行メモリ・キーワード抽出器を初期化 |
...
| **Process** | 1. `config` を解決（未指定なら `get_config()`）<br>2. `model_name` を解決（未指定なら `config.llm.model`）<br>3. `create_chat_client(config)` でLLMクライアントを生成<br>4. `config.memory.enabled` が真なら `create_execution_memory(config.memory.path)` で実行メモリ層を初期化（無効時は `None`）<br>5. `KeywordExtractor(prefer_mecab=True)` を初期化（失敗時は None） |
...
**概要**: LLM呼び出しなしで標準2ステップ計画（rag_search → reasoning）を生成する。実行メモリに十分な実績があれば rag_search の対象コレクションを優先コレクションに固定する。
...
| **Process** | 1. `_prioritized_collection(query)` で優先コレクションを取得（無ければ `None`）<br>2. step1=rag_search（`collection=優先 or None`, `fallback="web_search"`, `timeout_seconds=30`）を作成<br>3. step2=reasoning（`depends_on=[1]`）を作成<br>4. `ExecutionPlan` を構築し `plan_id` を採番 |
...
**概要**: LLMエラー時の安全な代替2ステップ計画を生成する。実行メモリの優先コレクションを最優先し、無ければ wikipedia 系コレクションを使用する。
...
|------|------|
| **Input** | `query: str` |
| **Process** | 1. `_prioritized_collection(query)` で優先コレクションを取得<br>2. 無ければ `_get_available_collections()` から wikipedia 系コレクションを選択（さらに無ければ None）<br>3. rag_search（`fallback="web_search"`）→ reasoning の2ステップを構築<br>4. `complexity=0.5` で `ExecutionPlan` を生成 |
...
classDef default fill:#000,stroke:#fff,color:#fff
classDef subgraphStyle fill:#1a1a1a,stroke:#fff,color:#fff
class PLANNER,CLIENT,CLAUDE,TYPES,QC,SCHEMAS,CONFIG,MEMORY,QSVC,PROMPTS,MECAB default

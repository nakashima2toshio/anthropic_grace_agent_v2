...
システムの特徴（ReAct + Reflection、フルスクラッチ実装、Gemini ネイティブ function-calling）です。
...
3. フルスクラッチ実装:
   　　google-genai SDK (function-calling) を直接利用し、柔軟な制御を実現しました。
   　　会話状態は genai の chat セッション (self.chat) が保持します（自前の messages リストは持ちません）。
4. スマート検索（キャッシュ + 並列検索）:
...
> **LLM の扱い（重要）:** プロジェクト標準 LLM は **Anthropic Claude (`claude-sonnet-4-6`)** ですが、
> 本書のレガシー `ReActAgent` は **Gemini ネイティブ function-calling** で動作します
> （既定 `gemini-2.5-flash`。非 gemini モデル名は `gemini-2.5-flash` へ自動回避）。
...
| ReActエージェント実行LLM | **Gemini ネイティブ function-calling**（既定 `gemini-2.5-flash`）   |
...
| エージェント対話 | 🤖       | **ReAct + Reflection Agent** (Gemini ネイティブ function-calling) との対話。ナレッジベース検索 + **Reflection (自己推敲)** による高品質な回答。 |
...
graph TD
    ...
    subgraph DataAccess ["データアクセス層"]
        LLM["google-genai SDK (ReAct LLM: gemini-2.5-flash 既定)"]
...
graph LR
    ...
    subgraph Helper_Layer ["Helpers"]
        GenAI["google-genai SDK (genai.Client / chats)"]
...
classDef default fill:#000,stroke:#fff,color:#fff
classDef subgraphStyle fill:#1a1a1a,stroke:#fff,color:#fff
class Main,InitUI,AgentPage,LogPage,OtherPages,AgentSvc,Tools,Cache,Parallel,QS,LogSvc,GenAI,HelperEmbed,QdrantWrapper default
...
| **サービス層**       | `services/agent_service.py`         | **エージェント制御コア (`ReActAgent`)**。ReActループ、Reflection、genai チャットセッション管理 |
...
| **LLMクライアント**  | `google-genai` SDK（直接利用）      | `genai.Client` / `client.chats.create` / `chat.send_message`（`helper/helper_llm.py` は経由しない） |
...
graph TB
    ...
    subgraph External ["External"]
        GenAI["Gemini LLM (gemini-2.5-flash / google-genai chat)"]
    ...
    Entry --> AgentUI
    AgentUI --> ReAct
    ReAct -- "chat.send_message + tools + system_instruction" --> GenAI
    GenAI -- "candidates[].content.parts (function_call)" --> ReAct
    ReAct -- "Execute" --> Tools
    Tools -- "embed_query" --> Gemini
    Tools -- "Search" --> Qdrant
    Qdrant -- "Documents" --> Tools
    Tools -- "function_response" --> ReAct
    ReAct -- "Part.from_function_response" --> GenAI
    GenAI -- "Final Answer" --> ReAct
    ReAct --> AgentUI
classDef default fill:#000,stroke:#fff,color:#fff
classDef subgraphStyle fill:#1a1a1a,stroke:#fff,color:#fff
class Entry,AgentUI,ReAct,Tools,GenAI,Gemini,Qdrant default
...
*   **クラス `ReActAgent`**:
    *   **LLMクライアント**: `_setup_client()` が `google-genai` の `genai.Client(api_key=...)` を **直接** 生成（API キーは `api.google_api_key` / `GEMINI_API_KEY` / `GOOGLE_API_KEY`）。`AnthropicClient` は経由しません。
    *   **チャットセッション**: `_create_chat()` が `client.chats.create(model=self.model_name, config=types.GenerateContentConfig(system_instruction=..., tools=[search_rag_knowledge_base, list_rag_collections]))` で生成し、`self.chat` に保持。会話状態は genai のチャットセッションが保持します（自前の `messages` リストは持ちません）。
    *   **モデル解決**: `model_name or get_config("models.default", "gemini-2.5-flash")`。`gemini` で始まらないモデル名（例: `claude-sonnet-4-6`）は `get_config("models.legacy_default", "gemini-2.5-flash")` へ自動回避し、`logger.warning` を出します。
    *   **ReActループ** (`_execute_react_loop`): 思考(Thought)と行動(Action=function_call)のサイクルを回し、ツール実行を制御。
    *   **Reflection** (`_execute_reflection_phase`): 回答案生成後の自己評価・修正フェーズを実行。
    ...
    *   **状態**: `self.client` / `self.chat` / `self.thought_log` / `self.keyword_extractor` / `self.session_id` / `self.use_hybrid_search`。
...
> `_create_chat()` で Python 関数オブジェクトのまま `tools_list = [search_rag_knowledge_base, list_rag_collections]` として
> `types.GenerateContentConfig(tools=...)` に渡されます（google-genai の Automatic Function Calling / 関数シグネチャから自動的にスキーマ化）。
> Anthropic 形式の `input_schema` dict や `_build_tools()` は存在しません。
> ただし実行時、`search_rag_knowledge_base` への `function_call` は `ReActAgent._execute_react_loop` 内で
> `search_rag_knowledge_base_cached` に振り替えられます（キャッシュ + 並列検索を適用するため）。
> `collection_name` は関数シグネチャ上は受け取りますが、`search_rag_knowledge_base` / `_cached` 側で実質的に全コレクション検索に回されます。
...
* **機能**: Gemini ネイティブ function-calling を用いた `ReActAgent` とのチャットインターフェース。
* **特徴**:
  * **モデル選択**: サイドバーで `GeminiConfig.AVAILABLE_MODELS`（`gemini-2.5-flash` 他）から使用モデルを選択可能。既定は `AgentConfig.MODEL_NAME`（= `GeminiConfig.DEFAULT_MODEL` = `gemini-2.5-flash`）。
...
Gemini ネイティブの function-calling 機能を利用し、以下のサイクルを回します。
![ReAct_fig](doc/assets/agent_5_react.png)

1. **ReAct フェーズ (解決) — `_execute_react_loop`**:

   * **Thought (思考)**: ユーザーの入力に対し、外部知識が必要か、どんなクエリで検索すべきか考える。
   * **Action (行動)**: ツール (`search_rag_knowledge_base`) を呼び出す（genai 応答の `candidates[0].content.parts` を走査し、`part.function_call` の有無で判定）。
   * **Observation (観察)**: ツールを実行し、その結果を `types.Part.from_function_response(name=..., response={'result': ...})` として `chat.send_message` で genai に返送。
   * **Draft Answer (ドラフト作成)**: `function_call` を含まないターンのテキスト（`current_turn_text_from_model`）を回答案 (`final_text`) とする。
...
| **Process** | 1. `self.thought_log` をリセット（会話状態は `self.chat` セッションが保持）。<br>2. **ReAct Loop** (`_execute_react_loop`):<br>　a. （任意）`KeywordExtractor` でキーワード抽出し入力を拡張。<br>　b. `self.chat.send_message(message=augmented_input)` を呼ぶ。<br>　c. `candidates[0].content.parts` を走査。`part.function_call` があればツール実行（`search_rag_knowledge_base` → `search_rag_knowledge_base_cached`）。<br>　d. 結果を `types.Part.from_function_response(...)` として `chat.send_message` で返送し、`max_turns`(既定10) まで反復。<br>　e. `function_call` がなければそのテキストを `final_text` イベントとしてYield。<br>3. **Reflection Phase** (`_execute_reflection_phase`):<br>　a. `REFLECTION_INSTRUCTION` + 回答案を `chat.send_message` で送信。<br>　b. 応答テキストを抽出し（`function_call` は防御的に無視）、`Final Answer:` で分割。<br>　c. `Final Answer:` 以降を最終回答とする。<br>4. `_format_final_answer` で整形し `final_answer` イベントをYield。 |
...
| `tool_call`     | `name`, `args`       | `function_call` 検出時          | 思考プロセスに追記し `st.spinner` で実行中表示         |
...
本ドキュメントは、ReAct + Reflection エージェントの理論的背景（概念図）と、`agent_rag.py` および関連モジュールの実装詳細を体系的にまとめたリファレンスです。本エージェントの推論ロールは Gemini ネイティブ function-calling（google-genai SDK 直接利用、既定 `gemini-2.5-flash`）で実行されます。

---

## 第2部: アーキテクチャ概念 (Theoretical Architecture)

エージェント (Gemini) の思考プロセスは、大きく2つのフェーズ（解決と推敲）で構成されています。
...
最終的な検索クエリの決定は **LLM (Gemini) モデル自体** が、システムプロンプトの指示に基づき「入力文」を解釈し「最適な検索クエリ」へと変換（推論）します。
...
* コード処理 (`ReActAgent.execute_turn` → `_execute_react_loop`):
  この文字列は（必要に応じて抽出キーワードを付与した上で）`self.chat.send_message(message=augmented_input)` 経由で
  google-genai のチャットセッションに送信されます。
  同時に、システムプロンプト (`SYSTEM_INSTRUCTION_TEMPLATE`、`_create_chat` で `system_instruction` として設定) によって、モデルには思考のルールが与えられています。

  > 指示: 「Thought: [なぜ検索が必要か、どんなクエリで検索するか]」

2. 生成・推論フェーズ (Gemini / google-genai 内部)
...
* コード処理 (`_execute_react_loop` → `search_rag_knowledge_base_cached`):
  google-genai から返ってきた `part.function_call` 情報（モデルが決めたクエリ。`fn.name` / `dict(fn.args)`）を Python 側で受け取り、検索関数を実行します。
  なお `collection_name` はモデルが指定しても無視され、全コレクションが検索対象になります。
...
つまり、「クエリ生成ロジック」の実体は主に LLM の推論プロセスの中にあります（`KeywordExtractor` は補助）。
...
2. **推敲フェーズ** (Reflection)
   ...
   * **コード処理** (`_execute_reflection_phase`): `REFLECTION_INSTRUCTION` + ドラフト回答を `reflection_msg` として `self.chat.send_message(...)` で同一チャットセッションに送信します（チャットセッションが ReAct ループの検索結果・思考ログを含む会話コンテキストを保持）。応答に `function_call` が含まれても防御的に無視します。
...
> **実装メモ:** 現行実装では Reflection は1回（シングルパス）です。`chat.send_message(REFLECTION_INSTRUCTION + ドラフト)` で評価・修正を行い、
...
* **`ReActAgent.__init__(selected_collections, model_name=None, session_id=None, use_hybrid_search=True)`**:
  * **役割**: エージェントを初期化します。`_setup_client()` が `genai.Client(api_key=...)` を生成し（`self.client`）、`_create_chat()` が `client.chats.create(model=..., config=types.GenerateContentConfig(system_instruction=..., tools=[...]))` でチャットセッション (`self.chat`) を生成します。
  * **詳細**: `model_name` 未指定時は `get_config("models.default", "gemini-2.5-flash")` を使用。`gemini` で始まらないモデル名は `get_config("models.legacy_default", "gemini-2.5-flash")` へ自動回避（警告ログ）。UI で選択した Gemini モデルを動的に切替可能。
  * **状態**: `self.client` / `self.chat`（会話状態を保持）/ `self.thought_log` / `self.keyword_extractor` / `self.session_id` / `self.use_hybrid_search` を保持。
...
google-genai の function-calling 機能と Python の `while turn_count < max_turns:` ループを組み合わせて ReAct を実現しています。
...
flowchart TD
    ...
    subgraph Agent_Process ["execute_turn メソッド"]
        direction TB
        Start["開始: messages リセット"] --> ReAct_Phase

        subgraph ReAct_Phase ["Phase 1: _execute_react_loop"]
            Think["思考 Thought"] --> Decide{"part.function_call あり?"}
            Decide -- Yes --> Action["行動: function_call を実行"]
            Action --> Observe["観察: function_response を send_message で返送"]
            Observe --> Think
        ...
        subgraph Reflection_Phase ["Phase 2: _execute_reflection_phase"]
            Review["REFLECTION_INSTRUCTION を send_message"] --> Critique["自己評価 & 修正"]
...
* **コード対応**: `_execute_react_loop` 内の `while turn_count < max_turns:` ループ（`max_turns` は `get_config("agent.max_turns", 10)`）。
* **ツール検出**: `current_response.candidates[0].content.parts` を走査し、`hasattr(part, 'function_call') and part.function_call` で判定（`function_call_found` フラグ）。`function_call` がないターンで `break` してドラフトを確定します。
* **Thoughtの可視化**: テキスト (`part.text`) に `Thought:` または `考え:` を含む場合、`log` イベントとしてYieldし Streamlit UI (`st.expander`) にリアルタイム表示します。
...
> **ツール定義 (`_create_chat`)**: Python 関数オブジェクトをそのまま `tools_list = [search_rag_knowledge_base, list_rag_collections]` として
> `types.GenerateContentConfig(tools=tools_list)` に渡します。google-genai が関数シグネチャと docstring からツールスキーマを自動生成します
> （Anthropic 形式の `input_schema` dict や `_build_tools()` は存在しません）。`collection_name` は関数シグネチャに残りますが、実行時は全コレクション検索に回されます。
...
sequenceDiagram
    ...
    participant LLM as "Gemini (google-genai chat)"
    participant Tool as "Agent Tools (Cached + Parallel)"

    Note over UI, LLM: Phase 1: ReAct Loop
    UI->>Svc: execute_turn(ユーザー入力)
    Svc->>LLM: chat.send_message(augmented_input)
    loop 解決するまで繰り返し (max_turns)
        LLM-->>Svc: candidates[].content.parts (text / function_call)
        alt part.function_call あり
            ...
            Svc->>LLM: chat.send_message(Part.from_function_response)
    ...
    Svc->>LLM: chat.send_message(REFLECTION_INSTRUCTION + ドラフト)
...
| 項目                   | 実装詳細                                  | 役割                                                                                     |
| :--------------------- | :---------------------------------------- | :--------------------------------------------------------------------------------------- |
| **ループ制御**         | `while turn_count < max_turns:`           | 思考・行動サイクルの維持と無限ループ防止（`max_turns` 既定10）。                          |
| **ツール検出**         | `hasattr(part, 'function_call') and part.function_call` | モデルがツール利用を要求した場合、対応する Python 関数 (`agent_tools.py`) を実行します。 |
| **結果フィードバック** | `chat.send_message(types.Part.from_function_response(name=..., response={'result': ...}))` | ツール実行結果を function_response として返送し、次のターンの推論に渡します（会話状態は chat セッションが保持）。 |
| **LLMクライアント**    | `self.client = genai.Client(...)` / `self.chat = client.chats.create(...)` | google-genai SDK 直接利用。応答は `response.candidates[0].content.parts`（`.text` / `.function_call`）を走査して解析します。 |
...
```bash
# ReAct エージェントの LLM 用 + Gemini Embedding 用（必須）
# ReActAgent は google-genai を直接利用するため、Gemini/Google のキーが必須です。
GEMINI_API_KEY=your_gemini_api_key
GOOGLE_API_KEY=your_gemini_api_key

# プロジェクト標準 LLM (Anthropic Claude) 用（GRACE 本体などで使用）
...
> **注:** 本書の `ReActAgent` は `api.google_api_key` → `GEMINI_API_KEY` → `GOOGLE_API_KEY` の順でキーを解決し、
> いずれも無ければ `ValueError` を送出します。`ANTHROPIC_API_KEY` は本エージェントの動作には使われません。
...
| 定数 / 設定                              | 場所                  | 値 / 既定                       | 用途                                   |
| :--------------------------------------- | :-------------------- | :------------------------------ | :------------------------------------- |
| `GeminiConfig.DEFAULT_MODEL`             | `config.py`           | `gemini-2.5-flash`              | 既定の ReAct LLM モデル                |
| `GeminiConfig.AVAILABLE_MODELS`          | `config.py`           | `gemini-2.5-flash` / `gemini-2.5-pro-preview` / `gemini-2.5-flash-preview` / `gemini-2.0-flash` | UI のモデル選択肢 |
...
| `models.legacy_default`                  | `get_config(...)`     | `gemini-2.5-flash`              | 非 gemini モデル名のフォールバック先   |
...
> **モデル名について:** Gemini モデル名はそのまま使用します（マッピングは作りません）。`ReActAgent` は
> 渡されたモデル名が `gemini` で始まるかどうかだけを判定し、始まらない場合（例: `claude-sonnet-4-6`）は
> Gemini エンドポイントでの失敗を避けるため `gemini-2.5-flash` へ自動回避します。
...
CLI 版エージェント（`agent_main.py` の `UpgradedCLIAgent.execute_turn`）も利用可能です。CLI 版も同じく google-genai を直接利用し（`genai.Client` / `chats.create` / `send_message`）、`AgentConfig.MODEL_NAME`（= `gemini-2.5-flash`）を既定モデルとします。ただし UI 版 `ReActAgent` と異なり、ストリーミングではなく最終回答文字列を返し、`search_rag_knowledge_base`（非キャッシュ版）を直接呼びます。
...
graph TD
    ...
    subgraph External_API ["External"]
        GeminiLLM["Gemini LLM API (gemini-2.5-flash)"]
...
classDef default fill:#000,stroke:#fff,color:#fff
classDef subgraphStyle fill:#1a1a1a,stroke:#fff,color:#fff
class AgentPage,AgentSvc,Tools,Cache,Parallel,QdrantSvc,Config,GenAI,HelperEmbed,Wrapper,GeminiLLM,Gemini,QdrantDB default

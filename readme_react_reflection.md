...
3. フルスクラッチ実装:
   　　Anthropic API (Tool Use) を直接利用し、柔軟な制御を実現しました。
   　　会話履歴は messages リストで自前管理（Anthropic はステートレス設計）。
4. スマート検索（キャッシュ + 並列検索）:
...
**一言で言うと**: Anthropic Claude活用型RAG Q&A生成・Qdrant管理、および **ReAct + Reflection 型エージェント** による対話を実現する統合Streamlitアプリケーション

**役割**:

- データ取得からベクトル検索までの **RAGパイプライン全体** を管理
- **ReAct + Reflection エージェント** を介した、ツール利用による高度な対話機能
- **LLM** は **Anthropic Claude** (`claude-sonnet-4-6` 既定)、**Embedding** は **Gemini** (`gemini-embedding-001` / 3072次元) を採用
...
| ファイル名     | agent_rag.py                                    |
| フレームワーク | Streamlit                                       |
| LLM            | Anthropic Claude (`claude-sonnet-4-6` 既定)     |
...
| エージェント対話 | 🤖       | **ReAct + Reflection Agent** (Claude Sonnet) との対話。ナレッジベース検索 + **Reflection (自己推敲)** による高品質な回答。 |
...
graph TD
    ...
    subgraph DataAccess ["データアクセス層"]
        LLM["Anthropic API (LLM: claude-sonnet-4-6)"]
...
graph LR
    ...
    subgraph Helper_Layer ["Helpers"]
        HelperLLM["helper/helper_llm.py (AnthropicClient)"]
...
classDef default fill:#000,stroke:#fff,color:#fff
classDef subgraphStyle fill:#1a1a1a,stroke:#fff,color:#fff
class Main,InitUI,AgentPage,LogPage,OtherPages,AgentSvc,Tools,Cache,Parallel,QS,LogSvc,HelperLLM,HelperEmbed,QdrantWrapper default
...
| **サービス層**       | `services/agent_service.py`         | **エージェント制御コア (`ReActAgent`)**。ReActループ、Reflection、履歴管理 |
...
| **LLM抽象化**        | `helper/helper_llm.py`              | `AnthropicClient` / `create_llm_client("anthropic")`              |
...
graph TB
    ...
    Entry --> AgentUI
    AgentUI --> ReAct
    ReAct -- "messages + tools + system" --> Claude
    Claude -- "Tool Use (stop_reason=tool_use)" --> ReAct
    ReAct -- "Execute" --> Tools
    Tools -- "embed_query" --> Gemini
    Tools -- "Search" --> Qdrant
    Qdrant -- "Documents" --> Tools
    Tools -- "tool_result" --> ReAct
    ReAct -- "tool_result" --> Claude
...
1. データDL -> 2. 前処理（プロンプト最適化、チャンク） -> 3. QA生成 (Claude) -> 4. 埋め込み登録 (Gemini Embedding) -> **5. エージェントによる活用 (Search & Answer)**
...
*   **クラス `ReActAgent`**:
    *   **LLMクライアント**: `create_llm_client("anthropic", default_model=...)` で `AnthropicClient` を生成。
    *   **会話履歴管理**: Anthropic はステートレスのため、`self._messages` リストで履歴を自前管理（`execute_turn()` の先頭でリセット）。
    *   **ReActループ** (`_execute_react_loop`): 思考(Thought)と行動(Action=Tool Use)のサイクルを回し、ツール実行を制御。
...
**責務**: テキストチャンクからQ/Aペアを生成するビジネスロジック (同期/非同期)。主な関数: `run_advanced_qa_generation`, `generate_qa_pairs`, `save_qa_pairs_to_file`。LLM は Anthropic Claude を使用。
...
> **注:** LLM に公開されるツールは `search_rag_knowledge_base` と `list_rag_collections` の2つ（`ReActAgent._build_tools()` の Tool Use 定義）。
> ただし実行時、`search_rag_knowledge_base` への呼び出しは `ReActAgent._execute_react_loop` 内で `search_rag_knowledge_base_cached` に振り替えられます（キャッシュ + 並列検索を適用するため）。
...
* **特徴**:
  * **モデル選択**: サイドバーで `ModelConfig.AVAILABLE_MODELS` から使用モデルを選択可能。
  ...
  * **イベント駆動**: `agent.execute_turn(prompt)` のジェネレータを反復し、`log` / `tool_call` / `tool_result` / `final_answer` イベントを処理。
...
Anthropic の Tool Use 機能を利用し、以下のサイクルを回します。
![ReAct_fig](doc/assets/agent_5_react.png)

1. **ReAct フェーズ (解決) — `_execute_react_loop`**:

   * **Thought (思考)**: ユーザーの入力に対し、外部知識が必要か、どんなクエリで検索すべきか考える。
   * **Action (行動)**: ツール (`search_rag_knowledge_base`) を呼び出す（Anthropic 応答の `stop_reason == "tool_use"` で判定）。
   * **Observation (観察)**: ツールを実行し、その結果を `tool_result` ブロックとして会話履歴に追記。
   * **Draft Answer (ドラフト作成)**: ツール呼び出しがなくなったターンのテキストを回答案とする。
...
| **Process** | 1. `self.thought_log` と `self._messages` をリセット。<br>2. **ReAct Loop** (`_execute_react_loop`):<br>　a. （任意）`KeywordExtractor` でキーワード抽出し入力を拡張。<br>　b. `self.llm.generate_with_tools(messages, tools, system, max_tokens)` を呼ぶ。<br>　c. `stop_reason == "tool_use"` かつ `tool_calls` があればツール実行（`search_rag_knowledge_base_cached`）。<br>　d. `tool_result` を `self._messages` に追記し、`max_turns` まで反復。<br>　e. ツール呼び出しがなければそのテキストを `final_text` としてYield。<br>3. **Reflection Phase** (`_execute_reflection_phase`):<br>　a. `REFLECTION_INSTRUCTION` + 回答案を `self._messages` に追記。<br>　b. `generate_with_tools(tools=[])` で自己評価を要求。<br>　c. `Final Answer:` 以降を抽出。<br>4. `_format_final_answer` で整形し `final_answer` イベントをYield。 |
| **Output**  | `Generator[Dict[str, Any], None, None]`: イベントストリーム<br>(例: `{'type': 'log', ...}`, `{'type': 'tool_call', ...}`, `{'type': 'tool_result', ...}`, `{'type': 'final_answer', ...}`)                                                                                                                                                                                                                            |
...
本ドキュメントは、ReAct + Reflection エージェントの理論的背景（概念図）と、ユーザーが選択可能な Claude（Anthropic）モデルを活用した `agent_rag.py` および関連モジュールの実装詳細を体系的にまとめたリファレンスです。
...
最終的な検索クエリの決定は **Claude モデル自体** が、システムプロンプトの指示に基づき「入力文」を解釈し「最適な検索クエリ」へと変換（推論）します。
...
* コード処理 (`ReActAgent.execute_turn` → `_execute_react_loop`):
  ...
  `self.llm.generate_with_tools(...)` 経由で Anthropic API (`messages.create`) に渡されます。
  同時に、システムプロンプト (`SYSTEM_INSTRUCTION_TEMPLATE`) によって、モデルには思考のルールが与えられています。
...
* コード処理 (`_execute_react_loop` → `search_rag_knowledge_base_cached`):
  Anthropic API から返ってきた `tool_use` 情報（モデルが決めたクエリ）を Python 側で受け取り、検索関数を実行します。
...
2. **推敲フェーズ** (Reflection)
   ...
   * **コード処理** (`_execute_reflection_phase`): `REFLECTION_INSTRUCTION` + ドラフト回答を `self._messages` に user メッセージとして追記し、`generate_with_tools(tools=[])`（ツールなし）で再度 Claude に送信します。これにより ReAct ループの検索結果・思考ログを含む会話コンテキストが維持されます。
...
> **実装メモ:** 現行実装では Reflection は1回（シングルパス）です。`generate_with_tools(tools=[])` で評価・修正を行い、
...
* **`ReActAgent.__init__(selected_collections, model_name=None, session_id=None, use_hybrid_search=True)`**:
  * **役割**: エージェントを初期化します。`create_llm_client("anthropic", default_model=model_name)` で `AnthropicClient` を生成。
  * **詳細**: `model_name` 未指定時は `get_config("models.default", "claude-sonnet-4-6")` を使用。UI で選択した Claude モデルを動的に切替可能。
  * **状態**: `self._messages`（会話履歴）、`self.system_instruction`、`self.tools`（Tool Use 定義）、`self.keyword_extractor` を保持。
...
Anthropic API の Tool Use 機能と Python の `for turn_count in range(1, max_turns + 1)` ループを組み合わせて ReAct を実現しています。
...
flowchart TD
    ...
    subgraph Agent_Process ["execute_turn メソッド"]
        ...
        subgraph ReAct_Phase ["Phase 1: _execute_react_loop"]
            ...
            Decide -- Yes --> Action["行動: tool_use を実行"]
            Action --> Observe["観察: tool_result を messages へ追記"]
            Observe --> Think
        ...
        subgraph Reflection_Phase ["Phase 2: _execute_reflection_phase"]
            Review["REFLECTION_INSTRUCTION 送信 (tools=[])"] --> Critique["自己評価 & 修正"]
...
* **ツール検出**: `result.stop_reason == "tool_use"` かつ `result.tool_calls` で判定（`result` は `ToolUseResponse` NamedTuple）。
* **Thoughtの可視化**: テキストに `Thought:` を含む場合、`log` イベントとしてYieldし Streamlit UI (`st.expander`) にリアルタイム表示します。
...
> **Tool Use 定義 (`ReActAgent._build_tools`)**: Anthropic 形式の dict リスト（`{"name", "description", "input_schema"}`）で構築されます。
> `search_rag_knowledge_base` の `input_schema` は `query`（必須）のみを公開し、`collection_name` は意図的に公開していません。
...
sequenceDiagram
    ...
    loop 解決するまで繰り返し (max_turns)
        LLM-->>Svc: ToolUseResponse (text + stop_reason + tool_calls)
        alt stop_reason == tool_use
            ...
            Svc->>LLM: generate_with_tools(messages + tool_result)
    ...
    Svc->>LLM: generate_with_tools(REFLECTION_INSTRUCTION, tools=[])
...
| **ツール検出**         | `result.stop_reason == "tool_use"`        | モデルがツール利用を要求した場合、対応する Python 関数 (`agent_tools.py`) を実行します。 |
| **結果フィードバック** | `self._messages.append({"role":"user","content":[tool_result...]})` | ツール実行結果を `tool_result` ブロックとして履歴に追記し、次のLLM呼び出しに渡します。 |
| **LLMクライアント**    | `self.llm.generate_with_tools(...)`       | `AnthropicClient`。戻り値は `ToolUseResponse` NamedTuple（`.text` / `.tool_calls` / `.stop_reason` / `.assistant_message`）。 |
...
| 定数 / 設定                              | 場所                  | 値 / 既定                       | 用途                                   |
| :--------------------------------------- | :-------------------- | :------------------------------ | :------------------------------------- |
| `ModelConfig.DEFAULT_MODEL`              | `config.py`           | `claude-sonnet-4-6`             | 既定LLMモデル                          |
| `ModelConfig.AVAILABLE_MODELS`           | `config.py`           | Claude 各モデル                 | UI のモデル選択肢                      |
...
> **モデル名について:** Claude モデル名はそのまま使用します（マッピングは作りません）。プロバイダーは
> `create_llm_client(provider)` の引数で明示指定し、内部のプロバイダー自動判定（`claude*` / `gemini*` / それ以外）に従います。
...
CLI 版エージェント（`agent_main.py` の `UpgradedCLIAgent.execute_turn`）も利用可能です。

### 8.2 典型的なワークフロー

1. RAGデータをダウンロード・前処理（📥 RAGデータDL）。
2. Q/A を生成（🤖 Q/A生成、Claude）。
...
classDef default fill:#000,stroke:#fff,color:#fff
classDef subgraphStyle fill:#1a1a1a,stroke:#fff,color:#fff
class AgentPage,AgentSvc,Tools,Cache,Parallel,QdrantSvc,Config,HelperLLM,HelperEmbed,Wrapper,Claude,Gemini,QdrantDB default

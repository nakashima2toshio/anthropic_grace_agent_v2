...
def create_no_info_judge(config) -> Callable[[str, str], Optional[bool]]:
    ...
    def judge(query: str, answer: str) -> Optional[bool]:
        prompt = (
            ...
            "次の回答が、質問の**中心的な事柄**に実質的に答えているかを判定してください。\n\n"
            "- answered : 中心的な事柄に具体的な情報（規定・手順・目安・条件など）を提供している。\n"
            "  補足で「弊社固有の情報は見当たらない」等と断っていても、本体が答えていれば answered。\n"
            "- no_info  : 中心的な事柄（例: 質問された日付・金額・可否そのもの）は「見つからない・\n"
            "  確認できない」に留まり、確認方法の案内や他窓口への誘導が主体になっている。\n\n"
            f"質問: {query}\n\n回答:\n{answer}\n\n"

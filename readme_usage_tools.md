...
入力テキスト/CSV（CSV は 1 行 = 1 文書として文書境界を保持）
  ...
  ↓ 上限強制分割（max-chunk-tokens 超過は文境界で分割）
チャンク CSV（2 ファイル + manifest）
```
...
| `--timestamp` | `false` | 出力ファイル名に日時サフィックスを付与（既定は固定名） |
| `--model` | `claude-sonnet-4-6` | 使用する LLM モデル（軽量用途は `claude-haiku-4-5-20251001` 推奨） |
| `--workers` | `8` | 並列ワーカー数（asyncio） |
| `--block-size` | `1000` | ブロックサイズ（文字数）。大きすぎると MAX_TOKENS エラー |
| `--max-chunk-tokens` | `512` | チャンクの最大トークン数。Embedding 入力上限 2048 未満を推奨 |
| `--continuity-mode` | `rule` | Step3 のモード（`rule` / `llm` / `off`） |
| `--text-column` | 自動検出 | CSV のテキストカラム名 |
| `--max-rows` | 全行 | 最大処理行数（CSV 用） |
| `--resume` | なし | チェックポイントから再開するジョブ ID |
| `--verbose` | `false` | 詳細ログ出力 |

> `--continuity-mode rule`（デフォルト）は Step3 を LLM 呼び出しなしのルールベース判定で実行します。`llm` を指定すると旧動作（隣接ペアごとに LLM 判定）になります。

### 2.4 出力ファイル

固定ファイル名（既定）で次の 3 ファイルが自動生成されます（`--timestamp` 指定時は `_YYYYMMDD_HHMMSS` を付与）:

| ファイル | 内容 |
|---------|------|
| `{入力名}_chunks.csv` | メタデータ付き（chunk_id, text, tokens, doc_id, sentence_count 等） |
| `{入力名}_chunks_simple.csv` | シンプル版（Text カラムのみ） |
| `{入力名}_chunks.manifest.json` | 後続ステージとの契約（カラム定義・カバレッジ・パラメータ） |
...
| `--analyze-coverage` | `false` | カバレッジ分析を実行（二重 Embedding コストに注意・オプトイン） |
...
> Embedding は Gemini `gemini-embedding-001`（3072 次元）に固定されています。プロバイダー選択オプションはありません。

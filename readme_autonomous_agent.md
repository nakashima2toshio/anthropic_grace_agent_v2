...
flowchart TB
    ...
    subgraph Planner_Layer["Planner Layer (grace/planner.py)"]
        ...
        PlanGeneration["LLM計画生成<br/>(Anthropic Claude / generate_structured)"]
    end
...
sequenceDiagram
    ...
    Planner->>Planner: 5. ルールベース計画 or LLM計画生成 (generate_structured / Anthropic Claude)
...
**概要**: ユーザーの質問を分析し、ExecutionPlan を生成する。**二層方式**を採用しており、ヒューリスティック複雑度が閾値未満の通常クエリは **ルールベースの2ステップ計画**（LLM呼び出しなし）を即時生成し、複雑なクエリや明示的なWeb検索指示の場合のみ **LLM計画生成** を行う。LLM呼び出しは `helper.helper_llm.create_llm_client("anthropic")` の `generate_structured()`（Anthropic Tool Use による構造化出力）経由で行い、Geminiの自動関数呼び出し（AFC）バグ回避コードは移植時に撤去済み。
...
| **Process** | 1. `estimate_complexity(query)` — キーワードベースのヒューリスティック複雑度を算出<br>2. `_should_use_llm_plan(query, complexity)` で分岐（`force_llm_plan` / Web検索マーカー / `complexity >= planner.llm_plan_complexity_threshold`(0.7)）<br>3. 分岐先=LLM計画: `_create_llm_plan()` が `PLAN_GENERATION_PROMPT` を構築し `_generate_plan_with_retry()` を呼ぶ（`generate_structured(response_schema=ExecutionPlan, max_tokens=8192, ...)`、一時的エラーは指数バックオフでリトライ）<br>4. 分岐先=ルールベース: `_create_rule_based_plan()` が LLM 呼び出しなしで標準2ステップ計画を生成<br>5. 複雑度は **構造化出力の `plan.complexity` をそのまま採用**（旧実装の別途LLM推定は統合・廃止）<br>6. `validate_plan_dependencies()` で依存関係を検証（警告のみ）<br>7. 計画IDを付与（`create_plan_id()`） |
| **Output**  | `ExecutionPlan` — 実行計画（LLM計画生成失敗時はルールベースのフォールバック計画を返却） |

**ルールベース計画 / フォールバック動作**: `_create_rule_based_plan()` は `rag_search`（`collection=None`＝全コレクション網羅、`fallback="web_search"`）→ `reasoning` の標準2ステップ計画を生成します。`_create_fallback_plan()`（LLM計画生成失敗時）も同じ `_create_rule_based_plan(query, complexity=0.5)` に委譲します。LLM計画生成プロンプトでも「計画には web_search ステップを含めず rag_search の fallback に web_search を指定する（web_search は結果不十分時に Executor が動的実行する）」ルールを徹底しています。
...
| **Process** | 元の計画の完全なJSON（`model_dump_json`、created_at/plan_id 除外）とフィードバックをプロンプトに含め、`generate_structured(response_schema=ExecutionPlan, max_tokens=4096)` で構造化出力。新しいplan_idを付与 |
...
| `search_priority`      | `["cc_news_2per_anthropic", "cc_news", "livedoor", "japanese_text", "wikipedia_ja"]` | コレクション検索優先順位（3072次元 cc_news を最優先、wikipedia_ja はフォールバック） |

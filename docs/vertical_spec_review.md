...
（`agent_support_example.py:182-184, 350`）。具体的な誤爆例:
...
直接押し下げるため、誤爆率は §6 のメトリクスで常時計測すべきである。
...
特に (A) の誤爆例は、まず失敗するテストとして固定してから直すのが安全。
...
| P1-5 | 純関数の単体テスト追加 | `_answer_gate` / `_decide_action` / エスカレ語判定（誤爆例を含む）を `tests/` に追加 |

### P2: 小規模コア改修（数十行規模）

| # | 提案 | 内容 |
|---|---|---|
| P2-1 | `allowed_collections`（許可リスト型の検索限定） | `RAGSearchTool.execute()` と `config.qdrant` に追加（§3.2）。フォールバック漏れを塞ぐ |
| P2-2 | `config.llm.prompt_addendum` | `ReasoningTool._build_prompt()` のシステム指示部で連結（§3.1）。executor / ReAct 経路にも効く |
| P2-3 | `contradiction_th` の分離 | 矛盾判定を `confirm_th` から独立（§4-C） |
| P2-4 | 出典・groundedness の採択側報告 | Web 採択時の表示を採択側基準に（§4-D） |

### P3: KPI 評価スクリプト（§6 に詳細設計）
...
| # | 提案 | 内容 |
|---|---|---|
| P5-1 | **二段判定**（キーワード＋LLM 意図分類） | 第 1 段: 現行キーワードを「候補検出」に格下げ。第 2 段: 候補ヒット時のみ `claude-haiku-4-5-20251001` で意図分類（`question / request / incident`）し、`incident`（障害・被害・トラブル報告）のみ強制エスカレ、`request` のみアクション発火。追加コストはヒット時の haiku 1 呼び出しに限定される |
...
`docs/vertical_test_data.md` §4 の 4 カテゴリに、§4-A の誤爆を計測する
...
| バージョン | 変更内容 |
|-----------|---------|
| 1.0 | 初版作成。成果物（設計書 v0.4・テストデータガイド v1.0・実装）と grace コアの突合レビュー。主要結論 3 点（残タスク 2 件は既存フックでほぼ実現可能／キーワード部分一致の誤爆が最大リスク／KPI 評価は eval/ 再利用）、不整合 D1〜D4、懸念点 A〜G、改善提案 P1〜P5、KPI 評価設計（5 カテゴリ・メトリクス定義）、テストデータガイド追補、推奨ロードマップを記載 |

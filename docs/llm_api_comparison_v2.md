...
> **本プロジェクト（`anthropic_grace_agent`）の採用構成**: LLM は **Anthropic Claude（`claude-sonnet-4-6`）**、Embedding は **Gemini `gemini-embedding-001`（3072次元）**。
> 本ドキュメントは Anthropic / OpenAI / Gemini の 3 プロバイダーを横断比較する参照資料であり、Gemini / OpenAI の記述は比較目的で保持している。
...
| v2.1 | 2026-06-16 | 本プロジェクト（`anthropic_grace_agent`）の採用構成（LLM=Anthropic Claude `claude-sonnet-4-6` / Embedding=Gemini `gemini-embedding-001`）を冒頭に明記。3プロバイダー比較表は維持。 |

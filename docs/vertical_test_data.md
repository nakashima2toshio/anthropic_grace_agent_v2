...
| `docs/vertical_test_data.md` | ガイド | 本書（テストデータ準備＋成果物一覧） | v1.0 |
...
各業界で **4 カテゴリ**を用意すると全分岐を検証できる（プロファイルの `sample_queries` 拡張に相当）。
...
3. **§4 のテスト質問で 4 分岐を確認** → KPI 評価スクリプト（`agent_support_verticals.md` §8 の残タスク #3）へ発展。
